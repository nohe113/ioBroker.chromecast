    ...
    class ChromecastDevice {
        ...
        _createObjects() {
            ...
            let states = {
                ...
                'seekTime': {
                    name: channels.player.name + '.seekTime',
                    def: 0,
                    type: 'number',
                    read: true,
                    write: true,
        ...
        _updatePlayerStatus(pStatus, pPreviousStatus) {
            ...
            // seekTime
            this.setStateIfChanged(this._states.seekTime.name,
                {val: Math.floor(status.seekTime), ack: true},
                Math.floor(cachedStatus.seekTime ));
        ...
        _ioBrokerChange(id, state) {
            ...
            else if (id === states.seekTime.name) {
                this._mediaPlayer.seekPromise(state.val);
